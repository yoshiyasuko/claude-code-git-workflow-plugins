...
`main` の場合、AskUserQuestionツールで確認する：
...
- 「コミットしてPR作成」→ Skillツールで `git-workflow:commit` スキルを実行する。引数: `skip-push skip-post-hooks` に加え、create-pr の引数に `skip-pre-hooks` が含まれている場合は `skip-pre-hooks` も付与する
- 「PR作成を中止」→ 処理を終了
...
プロジェクトルートの `.claude/skill-hooks.md` を Read ツールで読み込み、`## create-pr` セクションの `post-pr` フックに指定されたスキルがあれば、「説明」列に記載された質問文と選択肢で AskUserQuestion ツールを使って確認し、肯定的な選択肢が選ばれた場合に Skill ツールで指定スキルを実行する。
